...
class Shortcode {
	...
	function team_shortcode( $atts ) {
		...
		if ( $scID && ! is_null( get_post( $scID ) ) ) {
			...
			if ( $link && ( $linkType == 'new_page' || $linkType == 'external_link' ) ) {
				$arg['target'] = $target;
			}
			...
			$html              .= "<div class='rt-container-fluid rt-team-container rt-pos-r ".esc_attr($containerClass)."' id='{$layoutID}' {$containerDataAttr}>";
			...
			if ( $teamQuery->have_posts() ) {
				...
				if ( $isCarousel ) {
					...
					$html .= '<div class="rt-carousel-holder ' . esc_attr( $carouselClass ) . '" data-options="' . esc_js( wp_json_encode( $sliderOptions ) ) . '"  dir="' . esc_attr( $rtlHtml ) . '">';
