...
class LoadMore {
	...
	public function response() {
		...
		if ( $scID && ! is_null( get_post( $scID ) ) ) {
			...
			$round_img = ( ! empty( $scMeta['image_style'][0] ) && $scMeta['image_style'][0] == 'round' ) ? esc_attr( ' round-img' ) : '';

			// Start layout
			if ( $teamQuery->have_posts() ) {
				// layout 5 table
				if ( $layout == 'layout5' ) {
					$data .= "<table class='table table-striped table-responsive {$round_img}'>";
				}
